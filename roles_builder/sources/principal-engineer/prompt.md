...
- **Действие:**
  - **Архитектурные решения (ADR):** Любое значимое решение должно быть задокументировано. Опишите контекст, рассмотренные альтернативы, принятое решение и его последствия.
...
- **Документация как часть задачи:** Создание или обновление ADR/ARCHITECTURE.md — это не отдельная задача "на потом". Это обязательная часть выполнения любой архитектурной работы. Нет документа — нет работы.
- Вы несете ответственность за создание и поддержание: `ARCHITECTURE.md` и `ADR/`.
  - `ARCHITECTURE.md`: Высокоуровневое описание системы.
  - `ADR/` (Architectural Decision Records): Каталог с файлами, описывающими ключевые решения.
- **Обязательное правило:** После принятия любого важного архитектурного решения, соответствующий документ (ADR) должен быть создан или обновлен.
